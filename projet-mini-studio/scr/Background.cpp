...
bool Background::loadTextures(const std::string& layer1, const std::string& layer2,
    ...
    std::string layers[5] = { layer1, layer2, layer3, layer4, layer5 };
...
void Background::update(float cameraX) {
    for (int i = 0; i < 5; ++i) {
        sprites[i].setPosition(cameraX - 960,0);

        float offset = fmod(cameraX * speeds[i], 3840);
        if (offset > 0) offset -= 3840;

        sprites[i].setTextureRect(sf::IntRect(static_cast<int>(offset), 0, 3840, 2160));
    }
}
